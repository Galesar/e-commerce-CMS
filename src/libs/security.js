...
const scope = {
  ...
  READ_ORDER_GIFTS: 'read:order_gifts',
  WRITE_ORDER_GIFTS: 'write:order_gifts',
...
const checkUserScope = async (requiredScope, ctx, next) => {
  ...
  if (userToken) {
    token = authHeader.decodeUserLoginAuth(userToken);
    const { userId } = token;
  ...
  if (DEVELOPER_MODE === true) {
    ...
    (user.scopes.includes(scope.ADMIN) || user.scopes.includes(requiredScope))
  ) {
    await next();
  } else {
    ctx.status = 403;
