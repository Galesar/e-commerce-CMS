...
async function changeUpdatedObject(object) {
  ...
  if (!object.price && !object.stock_price) {
    object.price = 0;
    object.stock_price = 0;
  }
  return object;
}
...
function getSort({ sortQuery = 'priceup' }) {
  switch (sortQuery) {
    case 'priceup':
      return { price: -1 };
    case 'pricedown':
...
class ProductApi {
  ...
  async addProduct(object) {
    try {
      ...
      const importData = await changeUpdatedObject(object);
      const data = await product.create(importData);
  ...
  async getProductList(object) {
    ...
    try {
      ...
      // data = getSort({ array: data, sortQuery: sort });
